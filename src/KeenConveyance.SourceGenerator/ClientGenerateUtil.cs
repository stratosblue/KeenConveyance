...
internal record struct CodeDescriptor(string HintName, string Code);
...
internal static class ClientGenerateUtil
{
    ...
    public static IEnumerable<CodeDescriptor> GenerateClientCode(ImmutableArray<ServiceGenerateInfo> clientGenerateInfos, string rootNamespace)
    {
        string CodePrefix = $$"""
                            // <Auto-Generated/>
...
""");
        ...
        yield return new("GeneratedClient.g.cs", builder.ToString());
...
""");
            ...
            for (int i = 0; i < generateInfo.Methods.Items.Length; i++)
            {
                ...
                var parameterString = string.Join(", ", parameters.Select(m => $"{m.ParameterSymbol.Type.ToFullyQualifiedNullableString()} {m.Name}"));
                ...
                {
                    executeRequestString = $"ExecuteRequestAsync<{methodGenerateInfo.ReturnType.Type.ToFullyQualifiedNullableString()}>";
                }
                else
                {
                    executeRequestString = $"ExecuteRequestAsync<{methodGenerateInfo.ReturnType.ValueType.ToFullyQualifiedNullableString()}>";
                }
...
$$"""
        /// <inheritdoc/>
        public virtual {{methodGenerateInfo.MethodSymbol.ReturnType.ToFullyQualifiedNullableString()}} {{methodGenerateInfo.Name}}({{parameterString}})
        {
...
""");
            ...
            yield return new($"{generateInfo.Alias}.g.cs", builder.ToString());
        }
    }
